...
- **`can-mirror`** — no data model at all. The library mirrors the element's own DOM (attributes, children, form state) to everyone, so **the state you share _is_ the DOM**. Drop it on a `<textarea>` or a `<ul>` and you're done.
- **`can-play`** — you own a piece of shared **`data`** and a callback that updates the DOM when it changes. The general-purpose escape hatch when you need **fine-grained control** over the shape of your state.
...
In vanilla you have three tools, from least to most code: `can-mirror` (sync the DOM as-is), `can-play` (own a `data` model and update the DOM imperatively), and the experimental reactive `view` API (render the DOM declaratively from state).
