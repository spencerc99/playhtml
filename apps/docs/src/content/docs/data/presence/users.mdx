...
When cursors are enabled, `window.cursors.color` and `window.cursors.name` are aliases for these same fields — see the [Cursors](/docs/data/presence/cursors/#global-cursor-api) page. `playhtml.users` is the primary API; it does not require cursors.
