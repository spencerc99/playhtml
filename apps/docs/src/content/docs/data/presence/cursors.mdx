...
The `color` and `name` setters delegate to `playhtml.users.me`, which owns identity. See [Users](/docs/data/presence/users/).
...
For a reactive roster of everyone in the room, use [`useUsers()`](/docs/data/presence/users/).
