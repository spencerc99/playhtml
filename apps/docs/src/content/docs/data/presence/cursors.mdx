...
Listen for changes to cursor state. These fire for the same underlying changes as [`playhtml.users.onChange`](/docs/data/presence/users/); use whichever is more convenient — `users.onChange` delivers the whole roster, while these deliver one value per event.
...
These values automatically update when users join/leave or change their settings. They are the same data the users hooks return: prefer [`usePlayerIdentity()`](/docs/data/presence/users/) for your own identity and [`useUsers()`](/docs/data/presence/users/) for the roster — both work without cursors enabled.
