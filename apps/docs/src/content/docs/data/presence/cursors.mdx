...
Cursors expose a global `window.cursors` object.

`window.cursors` predates [`playhtml.users`](/docs/data/presence/users/). Its identity fields are the same data, not a second copy: `color` and `name` are aliases for `playhtml.users.me.color` and `playhtml.users.me.name`, and `allColors` is derived from the room's users. Reading or writing through either API gives identical results. We recommend `playhtml.users` for new code because it works whether or not cursors are enabled. `window.cursors` remains supported.

### Properties

```javascript
// Get all user colors (across the room).
// Same data as playhtml.users.getAll().map(u => u.color), deduplicated.
const colors = window.cursors.allColors;  // ["#3b82f6", "#8b5cf6", ...]

// Get/set your color (alias for playhtml.users.me.color)
window.cursors.color;  // "#3b82f6"
window.cursors.color = "#ff0000";

// Get/set your name (alias for playhtml.users.me.name)
...
Listen for changes to cursor state. These events fire for the same changes as [`playhtml.users.onChange`](/docs/data/presence/users/). The difference is shape: `users.onChange` delivers the whole roster, and these deliver one value per event.
...
These values update automatically when users join, leave, or change their settings. They are the same data the users hooks return. We recommend [`usePlayerIdentity()`](/docs/data/presence/users/) for your own identity and [`useUsers()`](/docs/data/presence/users/) for the roster; both work without cursors enabled.
