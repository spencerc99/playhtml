...
- `presence.playerIdentity` - User info `{ name, playerStyle: { colorPalette }, custom }`
...
`window.cursors.custom` and `window.cursors.setCustom(key, value, { persist })` are also available, as conveniences that delegate to `playhtml.users.me`. For the full custom-properties API (persistence semantics, the 1KB cap, and how properties reach cursor render hooks), see [Users](/docs/data/presence/users/).
...
For custom properties and a reactive roster of everyone in the room, use `useUsers()` and `usePlayerIdentity()` instead — see [Users](/docs/data/presence/users/).
...
### Status-Aware Cursors

Set a status on your own cursor and dim other users' cursors when they've gone away:
...
playhtml.init({
  cursors: {
    enabled: true,
    getCursorStyle: (presence) => {
      if (presence.playerIdentity.custom?.status === "away") {
        return { opacity: '0.3' };
      }
      return {};
    }
...
// Mark yourself away, e.g. on window blur. Session-only so a stale
// "away" status doesn't stick around on your next visit.
window.addEventListener('blur', () => {
  playhtml.users.me.setCustom('status', 'away', { persist: false });
});

window.addEventListener('focus', () => {
  playhtml.users.me.setCustom('status', 'active', { persist: false });
});
```
  </TabItem>
  <TabItem label="React">
...
<PlayProvider
  initOptions={{
    cursors: {
      enabled: true,
      getCursorStyle: (presence) =>
        presence.playerIdentity.custom?.status === "away"
          ? { opacity: '0.3' }
          : {}
    }
  }}
...
function AwayTracker() {
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('blur', setAway);
