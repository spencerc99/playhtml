...
import {
  ...
  canMoveRecipe,
  canToggleRecipe,
} from '@/components/playground/recipes/built-in-capabilities';
...
This page has live demos and both framework forms. For attribute names, data shapes, and options, see the [capabilities reference](/docs/reference/capabilities/).
...
`can-toggle` adds the `toggled` class when the element is on.
  </TabItem>
  <TabItem label="React">
```tsx
...
**What it does.** Shares hover state across everyone on the page. Presence-based, *not* persistent: when a user leaves, their hover clears.
