...
`can-mirror` syncs every change to the element — attributes, child nodes, and form state — to everyone, and saves them. You don't write any data or callbacks; just change the element with normal HTML, CSS, and JavaScript. Use it when the state you want to share is the DOM itself: a shared textarea, a form, a growing list of children.
