...
```
  ...
  <TabItem label="React">
```tsx
// CanDuplicateElement takes refs, not selectors. canDuplicateTo is the
// React equivalent of the can-duplicate-to attribute. See "React API".
import { CanDuplicateElement } from "@playhtml/react";

<CanDuplicateElement elementToDuplicate={templateRef} canDuplicateTo={penRef}>
