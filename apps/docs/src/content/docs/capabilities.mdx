...
Counters should use the mutator form of `setData`. It applies the increment to the draft at write time instead of copying a rendered `data` snapshot back into shared data. The same idea applies to pushing list entries, updating nested fields, and upserting keyed collections; see [Merging data changes](/docs/data/merging-data/).
