...
The **cursor** can go past the container edge — only the element's position is clamped. By default, 25% of the element stays inside the container on every edge so readers always have something to grab. Override that ratio with `can-move-bounds-min-visible`:

```html
<!-- Keep 50% of the magnet inside the fridge at all times -->
<div can-move can-move-bounds="fridge" can-move-bounds-min-visible="0.5" id="magnet">
  🧲
</div>
```

Use `can-move-bounds-min-visible="1"` to pin the element fully inside (legacy behavior), or `0` to let it slip out entirely.
