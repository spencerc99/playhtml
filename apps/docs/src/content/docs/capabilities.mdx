...
**What it does.** The "build your own capability" escape hatch. You define `defaultData` and either an imperative `updateElement` callback (shown below) or a declarative `view` that renders from state — playhtml syncs the data and re-renders. For anything beyond a one-liner, prefer the declarative path: see [**Custom elements**](/docs/custom-elements/) for `register` / `define` / `view` (vanilla) and `withSharedState` (React) side by side.
...
<Tabs syncKey="framework">
  <TabItem label="Vanilla HTML">
```html
<div id="cheer" can-play></div>

<script type="module">
  import { playhtml, html } from "playhtml";

  playhtml.register("cheer", {
    defaultData: { count: 0 },
...
Counters should use the mutator form of `setData`. It applies the increment to the draft at write time instead of copying a rendered `data` snapshot back into shared data. The same idea applies to pushing list entries, updating nested fields, and upserting keyed collections; see [Merging data changes](/docs/advanced/merging-data/).

Deeper material on custom capabilities lives in [**Custom elements**](/docs/custom-elements/) (the declarative `view` / `withSharedState` path), [Data essentials](/docs/data/data-essentials/), and [Dynamic elements](/docs/advanced/dynamic-elements/).
