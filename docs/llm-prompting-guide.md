...
- For SPA apps (React Router, Next.js, Astro ViewTransitions): pass `pathname` prop to `<PlayProvider pathname={pathname}>` so rooms and handlers stay in sync on navigation. Pass a `container` ref to `cursors` (or selector in vanilla) so cursor DOM survives body-swaps. See docs/navigation.md.
