...
its default. A channel handle held across the route change stays fully usable:
`setData` re-acquires its proxy (and re-attaches the observer, so `onUpdate`
keeps firing) instead of throwing, and a stale handle's `destroy` no longer
tears down a channel reopened in the new room.
