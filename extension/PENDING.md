...
released extension (extension/src/** and anything else that ships in the zip).
Changes that ship dark — behind a feature flag or the internal dev toggle — do
NOT get a bullet; add it in the PR that enables the feature for everyone.
Website (extension/website/**) and worker (extension/worker/**) changes deploy
on their own and do not get bullets. The release-prep workflow
