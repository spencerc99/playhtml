// ABOUTME: p5 instance-mode sketch — browsing journeys as comets flowing through a
// ABOUTME: turbulent field directly from origin to destination (no fixed cable rails).
// ABOUTME: Each path is uniquely bowed + flow-perturbed. Styles: channels/currents/ink.
...
interface Style {
  fieldAmp: number; // flow-field wander amplitude
  fieldScale: number; // spatial frequency of the field
  swing: number; // max per-journey bow (px) — point-to-point randomness
  seedJitter: number; // decorrelation of the field between journeys
...
const STYLES: Record<StyleName, Style> = {
  channels: { fieldAmp: 16, fieldScale: 0.004, swing: 55, seedJitter: 70, ink: false, buffer: false, bufFade: 0, tailMul: 1, headMul: 1 },
  currents: { fieldAmp: 30, fieldScale: 0.0026, swing: 105, seedJitter: 120, ink: false, buffer: true, bufFade: 0.1, tailMul: 1.4, headMul: 1.15 },
  ink: { fieldAmp: 40, fieldScale: 0.0019, swing: 140, seedJitter: 160, ink: true, buffer: true, bufFade: 0.06, tailMul: 2.1, headMul: 1.4 },
};
...
// ── geometry helpers ──────────────────────────────────────────────────────────
// Direct great-circle-ish arc between two points; longitude is unwrapped so the
// short way across the antimeridian is taken (the renderer wraps off-screen).
function arcPoints(a: GeoPoint, b: GeoPoint, n: number): GeoPoint[] {
  ...
  for (let i = 0; i <= n; i++) {
    const u = i / n;
    out.push({ lon: a.lon + dLon * u, lat: a.lat + (b.lat - a.lat) * u });
  }
  return out;
}
...
// Push interior points along the flow field; pin the endpoints so trips connect.
  ...
  new p5((p: P5Instance) => {
    ...
    function rebuild(): void {
      ...
      prepared = state.journeys.map((j) => {
        ...
        let pts = resample(
          arcPoints(j.origin, j.dest, ARC_SEGMENTS).map((g) => ({ x: proj.lonToX(g.lon), y: proj.latToY(g.lat) })),
          RESAMPLE_STEP,
        );

        // unique per-journey bow — the main source of point-to-point randomness
        const normals = normalsOf(pts);
        const swing = (hashFrac(seed * 3.13) - 0.5) * 2 * st.swing;
        for (let i = 1; i < pts.length - 1; i++) {
          const e = Math.sin(Math.PI * (i / (pts.length - 1)));
          pts[i].x += normals[i].x * swing * e;
          pts[i].y += normals[i].y * swing * e;
        }
        // flow-field wander layered on top
        pts = perturb(pts, st.fieldAmp, st.fieldScale, seedOff);

        return {
          path: buildScreenPath(pts),
    ...
    function drawTraveler(ctx: CanvasRenderingContext2D, t: Traveler, now: number, st: Style): boolean {
      ...
      const hr = (baseR * 0.72 + tr * 0.28) | 0;
      const hg = (baseG * 0.72 + tg * 0.28) | 0;
      const hb = (baseB * 0.72 + tb * 0.28) | 0;
